...
class Playlists extends _$Playlists {
  ...
  /// Флаг, указывающий, что в данный момент происходит загрузка плейлистов через API.
  bool _isLoadingFromAPI = false;

  /// Флаг, указывающий, что плейлисты уже были загружены через API хотя бы раз.
  ...
  Future<PlaylistsState?> build() async {
    ...
    // и загрузка ещё не производилась.
    if (connectivityManager.hasConnection && !_hasLoadedFromAPI && !_isLoadingFromAPI) {
  ...
  Future<void> _loadAPIPlaylists() async {
    ...
    try {
      // Загружаем пользовательские и рекомендуемые плейлисты по отдельности,
      // чтобы ошибка в одном не влияла на другой.
      final (List<ExtendedPlaylist>, int)? userPlaylists;
      final List<ExtendedPlaylist>? recommendedPlaylists;

      try {
        userPlaylists = await _loadUserPlaylists();
      } catch (error, stackTrace) {
        logger.e(
          "Failed to load user playlists via API",
          error: error,
      ...
      try {
        recommendedPlaylists = await _loadRecommendedPlaylists();
      } catch (error, stackTrace) {
        logger.e(
          "Failed to load recommended playlists via API",
          error: error,
          stackTrace: stackTrace,
        );
      ...
      // Если оба запроса не удались, то ничего не делаем.
      if (userPlaylists == null && recommendedPlaylists == null) return;
      ...
      final List<ExtendedPlaylist> allPlaylists = [
        if (userPlaylists != null) ...userPlaylists.$1,
        if (recommendedPlaylists != null) ...recommendedPlaylists,
      ];
